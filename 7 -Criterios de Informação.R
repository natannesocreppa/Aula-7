...
variacao_PIB <- read.table("c:/EconometriaA/Variacao.xls", header = T)  #Carrega os arquivos
...
cMA1 <- arima(var_PIB, order = c(0,0,1))   #Estima um modelo de médias móveis ordem q=1 , MA(1)
...
ARIbicma1BIC(AR1) #Extrai a estatística BIC Ddo modelo AR1
